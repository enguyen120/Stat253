...
```{r}
set.seed(321)
lifeexpectancy_cv10 <- vfold_cv(lifeExpectancyData, v = 11)
```
...
homes_rec <- recipe(Price ~ ., data = homes.orig) %>% #start with formula; y ~ . means include all predictors in the data set
    step_filter(Price >= 50000 & Price <= 500000) %>%
    ...
    step_unknown(all_nominal_predictors()) %>% # assign missing to "unknown" category
    step_mutate(
    ...
    step_cut(Rooms, breaks=c(0,4,8,10,20)) %>%
    step_relevel(Rooms,ref_level = '(4,8]') %>%
    step_log(Pct.College) %>%
    step_other(all_nominal_predictors()) %>% 
    step_dummy(all_nominal_predictors()) %>%
    step_interact(terms = ~ Living.Area:AnyFireplace) %>%
    step_center(Age) %>% #subtract mean
    step_normalize(Lot.Size,Land.Value) # both center and scale

homes_rec

homes_rec %>% 
  prep(training = homes.orig) %>% 
  bake(new_data = NULL)
homes_model_wf <- workflow() %>%
  add_recipe() %>% 
  add_model(lm_spec)
 
homes_model_wf

homes_fit_model <- homes_model_wf %>% 
  fit(data = )

homes_fit_model %>% tidy()
```
...
tune_res <- tune_grid( # new function for tuning hyperparameters
  lasso_wf_lifeex, # workflow
  resamples = lifeexpectancy_cv10, # folds
...
```{r}
# Code work for the first 4 stages: note alternate syntax of model formulas
# Can use . to specify all variables other than outcome
# Can use - to remove variables
