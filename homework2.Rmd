...
life_train <- training(lifeexpectancy_cv7$splits[[1]]) # pulls training data for the 1st split (1st fold is testing set)
#life_test <- testing(lifeexpectancy_cv7$splits[[1]]) # pulls testing data for the 1st split (1st fold is testing set)
data_split <- initial_split(lifeExpectancy, prop = 0.75) #Create Train/Test set
...
life_rec<-recipe(life_expec ~ ., data = life_train) %>%
...
train_prep %>%
  ...
  metrics(estimate = .pred, truth = life_expec)

lm_fit_train %>%
  tidy()

lm_fit_cv <- fit_resamples(life_model_wf, resamples =lifeexpectancy_cv7, metrics = metric_set(rmse, mae, rsq))
