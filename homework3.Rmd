...
data_split <- initial_split(health1, prop = 0.75, strata = platelets) #Create Train/Test set
health_train <- training(data_split) # Fit model to this
health_test  <- testing(data_split)
health_cv13 <- vfold_cv(health_train, v = 13, strata = platelets)
```
...
```{r}
## recipe and wf 
life_rec<-recipe(platelets ~ ., data = health_train) %>%
...
training_prep %>%
  ...
  metrics(estimate = .pred, truth = platelets)
