...
gam_wf <- workflow() %>%
    add_model(lm_spec) %>%
    add_recipe(gam_rec) %>%
  step_ns(high_blood_pressure, deg_free = 4) %>% 
     step_ns(age, deg_free = 4) %>%
     step_ns(anaemia, deg_free = 3) %>%
     step_ns(platelets, deg_free = 6) %>%
     step_ns(sex, deg_free = 4) %>%
     step_ns(serum_sodium, deg_free = 4) %>%
     step_ns(ejection_fraction, deg_free = 3)
```

```{r}
mod_gam <- fit_resamples(
    gam_wf,
    resamples = health_cv13, # cv folds
...
```{r}
mod_gam %>% pluck('fit') %>% summary() ## for some reason this isn't working :( 
